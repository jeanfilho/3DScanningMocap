...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		residuals[0] = T(LAMBDA) * T(m_weight) * (sourceTransformed[0] - targetTemp[0]);
		residuals[1] = T(LAMBDA) * T(m_weight) * (sourceTransformed[1] - targetTemp[1]);
		residuals[2] = T(LAMBDA) * T(m_weight) * (sourceTransformed[2] - targetTemp[2]);
